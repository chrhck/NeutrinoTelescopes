...
data = torch.load("/home/chrhck/repos/NeutrinoTelescopes/assets/models/checkpoint_10_nangrad.pt")
...
samples = samples[777:779, :].to("cpu")
labels = labels[777:779, :].to("cpu")
    ...
    def hook_cb(prev_fn, fn):
        def register_grad(grad_input, grad_output):
            ...
            if any(t is not None and torch.any(~torch.isfinite(t)) for t in grad_input):
                ...
                fn_dict[fn] = True
            if any(t is not None and torch.any(~torch.isfinite(t)) for t in grad_output):
                fn_dict[fn] = True
                #print(f"{fn} grad_input={grad_input} grad_output={grad_output}")
